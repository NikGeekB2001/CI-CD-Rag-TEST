...
import warnings
import re
from transformers import AutoTokenizer, AutoModelForCausalLM, AutoModelForQuestionAnswering, AutoModelForTokenClassification, pipeline
...
class MedicalAssistant:
    ...
    def load_models(self):
        try:
            ...
            self.text_model_name = "sberbank-ai/rugpt3large_based_on_gpt2"
            ...
            self.ner_base_model = AutoModelForTokenClassification.from_pretrained(self.ner_model_name, num_labels=13).to(self.device)
            
            try:
                self.ner_model = PeftModel.from_pretrained(self.ner_base_model, "./models/lora_adapter")
                print("   ✅ LoRA адаптер для NER загружен")
            except Exception as e:
                print(f"   ⚠️ LoRA адаптер не найден, используем базовую модель. Ошибка: {e}")
            ...
            # Создание генеративного pipeline
            self.text_generator = pipeline("text-generation", model=self.text_model, tokenizer=self.text_tokenizer)
    ...
    def analyze_entities(self, text: str) -> str:
        ...
        try:
            ...
            if not ner_results:
                return "Медицинские сущности не найдены."
            
            entities = [f"{res['word']} ({res['entity_group']})" for res in ner_results]
            return "Найденные медицинские сущности: " + ", ".join(entities)
        except Exception as e:
    ...
    def search_knowledge(self, query: str) -> str:
        ...
        try:
            return self.rag_pipeline.search(query)
        except Exception as e:
            return f"Ошибка при поиске информации: {e}"

    def process_question(self, question: str) -> str:
        if not self.models_loaded:
            return "Извините, модели не загружены. Попробуйте перезапустить приложение."

        entities = self.analyze_entities(question)
        context = self.search_knowledge(question)
        response = self.generate_generative_response(question, context, entities)
        return response
